  ...
  Xadmin.prototype.end = function() {
	...
	Xadmin.prototype.add_tab = function (title,url,is_refresh) {
		...
		for (var i = 0; i <$('.x-iframe').length; i++) {
		    console.log($('.x-iframe').eq(i).attr('tab-id')+"路径"+$('.x-iframe').eq(i).attr('src'))
            if($('.x-iframe').eq(i).attr('tab-id')==id ){
           //     console.log("重复")
                element.tabChange('xbs_tab',id);//切换页面
                if(is_refresh)
		...
		this.add_lay_tab(title,url,id);//添加新的top页面
...
layui.use(['layer','element','jquery'],function() {
    ...
    $('.left-nav #nav').on('click', 'li', function(event) {
        ...
        if($(this).parent().attr('id')=='nav'){//每次点击
